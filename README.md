...
| `vdprobe workspace-manager --confirm-mutate` | minimal manager self-test: registers real hotkeys (Ctrl+Alt+F9/F10) and a tray icon, dispatches WM_HOTKEY messages that drive one probe-owned monitor-local A1 -> A2 -> A1 switch, then unregisters and cleans up; no user HWNDs or native desktop lifecycle are touched |
| `vdprobe workspace-manager-test` | deterministic config parsing, hotkey binding validation, and dispatch resolution for the minimal manager; no real hotkey, window, COM, or native shell call is made |
